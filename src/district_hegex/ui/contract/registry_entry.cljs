(ns district-hegex.ui.contract.registry-entry
  (:require
    [cljs-solidity-sha3.core :refer [solidity-sha3]]
    [cljs-web3.eth :as web3-eth]
    [district-hegex.shared.utils :refer [vote-option->num]]
  ...
  (fn [{:keys [:db :store]} [{:keys [:reg-entry/address :vote/amount :vote/option :district/name]}]]
    (let [active-account (account-queries/active-account db)
          ...
          extra-data (web3-eth/contract-get-data (contract-queries/instance db :district address)
                                                 ...
                                                 amount
                                                 secret-hash)
          store-votes-args [[:district-hegex.ui.core/votes active-account address] {:option option :salt salt}]
...
(re-frame/reg-event-fx
  ...
  (fn [{:keys [:db :store]} [{:keys [:reg-entry/address :district/name]}]]
    (let [active-account (account-queries/active-account db)
          {:keys [option salt]} (get-in store [:district-hegex.ui.core/votes active-account address])
